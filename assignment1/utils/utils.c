...
int32_t read_file_to_buffer(char *name, char **msg_buf, int32_t *length)
{
	...
	{
		LOG_EXCEPTION("Could not open file. Does it exist?", fp, NEQ, NULL, TRUE);
		ret_val = -1;
		goto EXIT_LABEL;
	}
	/* Determine file size */
	...
	{
		LOG_EXCEPTION("Could not allocate sufficient buffer.", buffer, NEQ, NULL, FALSE);
		ret_val = -1;
		goto EXIT_LABEL;
	}
...
int32_t write_file_to_disk(char *name, char *msg_buf, int32_t length)
{
	...
	{
		LOG_EXCEPTION("Could not create file.", fp, NEQ, NULL, TRUE);
		ret_val = -1;
		goto EXIT_LABEL;
	}
